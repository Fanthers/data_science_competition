...
'''
利用在1w张原始cifar10上训练好的Resnet模型来生成cifar10的对抗样本
...
class MyDataset(torch.utils.data.Dataset):
    def __init__(self, transform, mode="train"):
        images = np.load('source/data.npy')
...
def main():
    ...
    for arch in ['resnet50']:
        ...
        model.load_state_dict(torch.load('./source/resnet50.pth.tar', map_location='cpu')['state_dict'])
        model = model.cuda()
        # Test
...
if __name__ == '__main__':
    ...
    # 保存生成的对抗样本用于下一步训练
    np.save('./datasets/data_adv_3w.npy', images_glob)
    np.save('./datasets/label_adv_3w.npy', labels_glob)